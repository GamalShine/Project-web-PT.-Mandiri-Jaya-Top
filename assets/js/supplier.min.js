// Inisialisasi DataTable untuk supplier
let url, supplier = $("#supplier").DataTable({
    ...
    columnDefs: [
        {
            searchable: false, // Salah ejaan telah diperbaiki
            orderable: false,
    ...
    columns: [
        ...
        { data: "alamat" }, // Kolom untuk alamat
        { data: "telepon" }, // Kolom untuk telepon
...
function remove(a) {
    Swal.fire({
        ...
        if (e.value) {
            $.ajax({
                url: removeUrl, // URL untuk menghapus data
                type: "post",
                dataType: "json",
                data: { id: a },
                success: () => {
...
function edit(a) {
    $.ajax({
        url: get_supplierUrl, // URL untuk mendapatkan data supplier
        type: "post",
        dataType: "json",
        data: { id: a },
        success: a => {
...
supplier.on("order.dt search.dt", () => {
    supplier.column(0, { search: "applied", order: "applied" }).nodes().each((a, e) => {
        a.innerHTML = e + 1;
    });
});
